    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "<span></span><a href='$mlink'>上一篇：{$preRow['title']}</a> ";
                $this->PreNext['preimg'] = "<a href='$mlink'><img src=\"{$preRow['litpic']}\" alt=\"{$preRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['pre'] = "<span>上一篇：没有了</span> ";
            ...
            if(is_array($nextRow))
            {
                ...
                $this->PreNext['next'] = "<span></span><a href='$mlink'>下一篇：{$nextRow['title']}</a> ";
                $this->PreNext['nextimg'] = "<a href='$mlink'><img src=\"{$nextRow['litpic']}\" alt=\"{$nextRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['next'] = "<span>下一篇：没有了 </span>";
